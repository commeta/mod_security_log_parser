...
## Описание скрипта modsec_recedive.sh

Этот скрипт отслеживает файлы журналов аудита ModSecurity и ведет учет атак, выявленных по IP-адресам. Если количество атак от одного IP превышает заданный порог, скрипт записывает это в отдельный журнал.

Особенности:

- Автоматическое создание журналов атак.
- Мониторинг директории журналов ModSecurity.
- Учет числа атак от одного IP-адреса.
- Запись в отдельный журнал, когда порог атак от одного IP достигнут.
- Периодическая очистка журнала атак.
